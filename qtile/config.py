...
terminal = "kitty" 
browser = "brave"
rofi_apps = "rofi -show drun"
rofi_tabs = "rofi -show window"
...
keys = [
    ...
    Key(
        [mod], "b", 
        lazy.spawn(browser), desc="Open My Browser"
    ),
    Key(
        [mod], "d", 
        lazy.spawn(rofi_apps)
    ),
    Key(
        [mod], "t", 
        lazy.spawn(rofi_tabs)
    ),
    ...
    # Key([], "XF86AudioMute", lazy.widget["volume"].toggle_mute())
]
...
screens = [
    Screen(
        ...
        wallpaper="/home/main/Pictures/Wallpapers/zukuna.jpg",
        wallpaper_mode="fill",
    ),
]
...
def autostart():
    ...
    subprocess.Popen(['/usr/local/bin/dunst'])
